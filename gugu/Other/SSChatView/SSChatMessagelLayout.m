...
-(void)setText{
    ...
    mTextView.userInteractionEnabled = NO;
    [mTextView sizeToFit];
...
//        if (self.message.messageFrom == SSChatMessageFromMe) {
//            
//        }
...
-(void)setImage{
    ...
    CGFloat imgWidth = SSChatImageMaxSize;
    CGFloat imgHeight = SSChatImageMaxSize;
    if (image) {
        imgWidth  = CGImageGetWidth(image.CGImage);
        imgHeight = CGImageGetHeight(image.CGImage);
    }
...
//    CGFloat changeLength = timeLength/60;
    CGFloat currentLength = SSChatVoiceMinWidth;
