...
#import "CB_ActivityChatController.h"
...
@property (nonatomic,strong) CB_AddressBottomView *bottomAddress;
@property (nonatomic,strong) CB_NAviTypeChooseView *chooseNaviView;
...
//    [self action_readMessage:self.model];
    ...
    self.chatViewType = CB_CHAT_TYPE_Chat;
...
-(void)action_configNavButtons{
    ...
    [mapBtn setImage:[UIImage imageNamed:@"last_go.png"] forState:UIControlStateNormal];
    UIBarButtonItem *mapItem = [[UIBarButtonItem alloc] initWithCustomView:mapBtn];
    
    self.navigationItem.leftBarButtonItems  = @[backItem,hxItem,mapItem];
...
//    }];
    ...
    NSArray *menuModelArr = @[ menuModel1,menuModel2,menuModel3,menuModel4];
    return menuModelArr;
}
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(action_playNext) name:NOTIFICATION_AUDIO_PLAY_FINISHED object:nil];
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(action_receiveRealtimeShare:) name:NOTIFICATION_SOCKET_POSITION_ONTIME object:nil];
...
-(void)initAction{
    ...
    self.bottomAddress.block_chooseAddress = ^(CB_MessageModel * _Nonnull msgModel) {
        [waekSelf action_showActivityAlert:msgModel];
    };
    self.chooseNaviView.block_zudui = ^(CB_MessageModel * _Nonnull model) {
        [waekSelf action_goActivityWith:model inWay:1];
    };
    self.chooseNaviView.block_guihua = ^(CB_MessageModel * _Nonnull model) {
        [waekSelf action_goActivityWith:model inWay:0];
    };
...
-(void)action_refreshHistory{
    ...
    [[NetWorkConnect manager] postDataWith:para withUrl:CHAT_MESSAGE_HISTORY withResult:^(NSInteger resultCode, id responseObject, NSError *error) {
        if(resultCode == 1){
            ...
            NSDictionary *lastMessage = [items lastObject];
            CB_MessageModel *model = [CB_MessageModel modelWithDictionary:lastMessage];
...
-(void)SSChatKeyBoardInputViewBtnClickFunction:(NSInteger)index{
    ...
    if (index== 14) {
        [self action_goMapSignal];
    }
}

-(void)action_goMapSignal{
...
-(void)sendMessage:(NSDictionary *)dic messageType:(SSChatMessageType)messageType{
    ...
    [SSChatDatas sendMessage:m_dict withImage:model.image sessionId:_sessionId messageType:messageType messageBlock:^(SSChatMessagelLayout *layout, NSError *error, NSProgress *progress) {
...
-(void)SSChatMapCellClick:(NSIndexPath *)indexPath layout:(SSChatMessagelLayout *)layout{
    [self action_showActivityAlert:layout.message.model];
}

-(void)action_showActivityAlert:(CB_MessageModel *)model{
    
    [UIView animateWithDuration:0.3 animations:^{
        self.bottomAddress.transform = CGAffineTransformIdentity;
    }completion:^(BOOL finished) {
        
    }];
    
    if ([model.SendUserId isEqualToString:[UserModel shareInstance].Guid]) {
        [self.chooseNaviView.btn_guihua setTitle:@"行程规划" forState:UIControlStateNormal];
    }else{
        [self.chooseNaviView.btn_guihua setTitle:@"个人导航" forState:UIControlStateNormal];
    }
    
    self.chooseNaviView.model = model;
    [self.chooseNaviView.img_navi sd_setImageWithURL:[NSURL URLWithString:model.FileUrlURL]];
    self.chooseNaviView.lbl_addressName.text = model.AddressName;
    self.chooseNaviView.lbl_brief.text = model.SubName;
    
    [self action_shownaviTypeAlertWith:model];

}

-(void)action_goActivityWith:(CB_MessageModel *)model inWay:(NSInteger)type{
    ...
    [[CB_LocationManager shareInstance] locateWithCompleted:^(NSString * _Nonnull formattedAddress, CLLocation * _Nonnull location) {
        ...
        if (type == 0) {
            ...
            //行程规划
            if ([model.SendUserId isEqualToString:[UserModel shareInstance].Guid]) {
                CB_ActivityChatController *page = [CB_ActivityChatController new];
                ...
                page.userCoordinate = location.coordinate;
                [self.navigationController pushViewController:page animated:YES];
            }else{
                //个人导航
                ...
                route.isFromChat = YES;
                route.isGroupNavi = NO;
                route.chatType = self.chatType;
                route.sessionId = self.sessionId;
                route.isGroup = self.isGroup;
                route.model = model;
                route.SendId = self.SendId;
                route.groupModel = self.groupModel;
                route.friendModel = self.friendModel;
                [CB_MessageManager action_saveNavi:model];
...
-(void)action_shownaviTypeAlertWith:(CB_MessageModel *)msgModel{
    [self.view_back removeFromSuperview];
    [[UIApplication sharedApplication].keyWindow addSubview:self.view_back];
    [[UIApplication sharedApplication].keyWindow addSubview:self.chooseNaviView];
    [UIView animateWithDuration:0.3 animations:^{
        self.view_back.alpha = 1;
        self.chooseNaviView.alpha = 1;
    }completion:^(BOOL finished) {
        
    }];
}

-(void)action_showBottomAddres{
    [[UIApplication sharedApplication].keyWindow addSubview:self.view_back];
    [[UIApplication sharedApplication].keyWindow addSubview:self.bottomAddress];
    [self.bottomAddress refreshData];
    [UIView animateWithDuration:0.3 animations:^{
        self.bottomAddress.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, 0, -self.bottomAddress.height);
        self.view_back.alpha = 1;
    }completion:^(BOOL finished) {
...
-(void)action_closeBack{
    [UIView animateWithDuration:0.3 animations:^{
        self.bottomAddress.transform = CGAffineTransformIdentity;
        self.chooseNaviView.alpha = 0;
...
-(void)action_receiveRealtimeShare:(NSNotification *)notification{
    CB_MessageModel *model = notification.object;
    [self.mapView action_updateUserWith:model];
//    if ([model.SendId isEqualToString:self.SendId]) {
//        [self.mapView action_updateUserWith:model];
//    }
}
...
//    [self action_stopTimer];
    ...
    self.shareTimer = [NSTimer scheduledTimerWithTimeInterval:6 target:self selector:@selector(action_joinShare) userInfo:nil repeats:YES];
    [self.shareTimer fire];
}
...
-(void)action_stopTimer{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFICATION_STOP_REALTIME_SHARE object:nil];
    [self action_exitShare];
    [self.mapView action_clearUsers];
}

-(void)action_joinShare{
    [self action_shareLocation:YES];
}
-(void)action_exitShare{
    [self action_shareLocation:NO];
    
}

-(void)action_shareLocation:(BOOL)isJoin{
...
//            NSDictionary *posDict = @{@"Lat":@(location.coordinate.latitude),@"Lng":@(location.coordinate.longitude)};
//            NSString *position = [posDict modelToJSONString];
//            [position stringByReplacingOccurrencesOfString:@"/" withString:@""];
            NSString *position = [NSString stringWithFormat:@"{Lat:%f,Lng:%f}",location.coordinate.latitude,location.coordinate.longitude];

            
            NSDictionary *dict = @{@"Messsage":position,@"ReceiveId":weakSelf.SendId,@"Join":@(isJoin)};
...
-(void)action_playNext{
    ...
    if (nextIndex == -1 || nextIndex>=self.datas.count) {
        self.isPlaying = NO;
        return;
    }
...
-(void)action_showlongGesture:(SSChatMessagelLayout *)layout{
    ...
    if ([layout.message.model.SendUserId isEqualToString:[UserModel shareInstance].Guid]) {
        [titles addObject:@"删除"];
    }
...
-(UIView *)view_back{
    if (!_view_back) {
        _view_back = [[UIView alloc] initWithFrame:self.view.bounds];
        _view_back.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.3];
        _view_back.alpha = 0;
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(action_closeBack)];
        [_view_back addGestureRecognizer:tap];
    }
    return _view_back;
}

-(CB_AddressBottomView *)bottomAddress{
    if (!_bottomAddress) {
        _bottomAddress = [[[NSBundle mainBundle] loadNibNamed:@"CB_AddressBottomView" owner:self options:nil] lastObject];
        _bottomAddress.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, 360+BottomPadding);
        _bottomAddress.sendId = self.SendId;
    }
    return _bottomAddress;
}

-(CB_NAviTypeChooseView *)chooseNaviView{
    if (!_chooseNaviView) {
        _chooseNaviView = [[[NSBundle mainBundle] loadNibNamed:@"CB_NAviTypeChooseView" owner:self options:nil] lastObject];
        _chooseNaviView.frame = CGRectMake((SCREEN_WIDTH-280)/2, (SCREEN_HEIGHT-200)/2, 280, 200);
        _chooseNaviView.alpha = 0;
    }
    return _chooseNaviView;
}
