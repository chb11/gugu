...
-(instancetype)initWithFrame:(CGRect)frame with:(BOOL)isShowRealTime{
    if(self = [super initWithFrame:frame]){
        ...
        if (!isShowRealTime) {
            ...
            titles = @[@"相册",@"拍照",@"位置",@"发起共享",@"自主导航"];
            images = @[@"相册",@"相机",@"位置",@"发起共享",@"自主导航"];
        }
...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(action_stopShare) name:NOTIFICATION_STOP_REALTIME_SHARE object:nil];
...
-(void)action_stopShare{
    UIView *shareView= [self viewWithTag:10+3];
    for (UIView *sub in shareView.subviews) {
