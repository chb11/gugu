...
- (void)playSongWithUrl:(NSString *)songUrl
{
    dispatch_async(dispatch_queue_create("playSoundFromUrl", NULL), ^{
        ...
        if ([songUrl containsString:@"http"]) {
...
//    NSError *playerError;
    _player = [[AVAudioPlayer alloc]initWithData:soundData error:nil];
    _player.volume = 5.0f;
    if (_player == nil){
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFICATION_AUDIO_PLAY_FINISHED object:nil];
}
