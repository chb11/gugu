...
-(instancetype)init{
    if(self = [super init]){
        ...
        self.frame = CGRectMake(0, SCREEN_Height-SSChatKeyBoardInputViewH-BottomPadding-NavBarHeight, SCREEN_Width, SSChatKeyBoardInputViewH);
...
-(void)setIsShowRealTime:(BOOL)isShowRealTime{
    _isShowRealTime = isShowRealTime;
    if (!_isShowRealTime) {
        if (_mKeyBordView) {
            [_mKeyBordView removeFromSuperview];
        }
        _mKeyBordView = [[SSChatKeyBordView alloc]initWithFrame:CGRectMake(0, self.height, SCREEN_Width, SSChatKeyBordHeight) with:NO];
        _mKeyBordView.delegate = self;
        [self addSubview:_mKeyBordView];
    }
}
    ...
    if(noti.name == UIKeyboardWillHideNotification){
        ...
        if(_keyBoardStatus == SSChatKeyBoardStatusSymbol ||
           _keyBoardStatus == SSChatKeyBoardStatusAdd){
            height = BottomPadding+SSChatKeyBordHeight;
        }
    }else{
        ...
        if(height==BottomPadding || height==0) height = _keyBoardHieght;
    }
    ...
    [UIView animateWithDuration:_changeTime animations:^{
        if(self.keyBoardStatus == SSChatKeyBoardStatusDefault ||
           self.keyBoardStatus == SSChatKeyBoardStatusVoice){
            self.bottom = SCREEN_Height-BottomPadding-NavBarHeight;
        }else{
            self.bottom = SCREEN_Height-self.keyBoardHieght-NavBarHeight;
        }
    } completion:nil];
    ...
    switch (self.keyBoardStatus) {
        ...
        case SSChatKeyBoardStatusDefault:{
            if(sender.tag==10){
                ...
                self.keyBoardHieght = BottomPadding+SSChatKeyBordHeight;
        ...
        case SSChatKeyBoardStatusSymbol:{
            ...
            if(sender.tag==10){
                ...
                _mKeyBordView.mCoverView.hidden = NO;
                self.mTextView.hidden = YES;
                self.keyBoardHieght = BottomPadding;
        ...
        case SSChatKeyBoardStatusAdd:{
            if(sender.tag==10){
                ...
                _mKeyBordView.mCoverView.hidden = YES;
                self.mTextView.hidden = NO;
                self.keyBoardHieght = BottomPadding+_mKeyBordView.height;
                _mKeyBordView.type = KeyBordViewFouctionSymbol;
            }else{
                [self.mTextView becomeFirstResponder];
                _mKeyBordView.mCoverView.hidden = YES;
    ...
    if(index==200){
        ...
        if(_delegate && [_delegate respondsToSelector:@selector(SSChatKeyBoardInputViewBtnClickFunction:)]){
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.delegate SSChatKeyBoardInputViewBtnClickFunction:index];
            });
        }
    }
}
...
- (void)beginRecordVoice:(UIButton *)button{
    ...
    NSDictionary *recordSetting = @{AVEncoderAudioQualityKey : [NSNumber numberWithInt:AVAudioQualityMin],
                                    AVEncoderBitRateKey : [NSNumber numberWithInt:44100],
    ...
    [_recorder record];
    _playTime = 0;
    _playTimer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(countVoiceTime) userInfo:nil repeats:YES];
    [UUProgressHUD show];
}
...
{
    [UUProgressHUD changeSubTitle:@"松手取消录制"];
}

- (void)RemindDragEnter:(UIButton *)button
{
    [UUProgressHUD changeSubTitle:@"上移取消录制"];
}
...
//    _playTime ++;
    ...
    if (_playTime>=59) {
        ...
        CGFloat volumn= [self.recorder peakPowerForChannel:0];
        [UUProgressHUD action_volumn:volumn];
    }
}
...
//回调录音资料
- (void)endConvertWithData:(NSData *)voiceData withFile:(NSString *)voicepath
{
    if(_delegate && [_delegate respondsToSelector:@selector(SSChatKeyBoardInputViewBtnClick:sendVoice:voicePath:time:)]){
        [self.delegate SSChatKeyBoardInputViewBtnClick:self sendVoice:voiceData voicePath:voicepath time:_playTime+1];
    }
...
//    [_audioSession setActive:YES withOptions:AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation error:nil];
    ...
    if (audioData) {
        [self endConvertWithData:audioData withFile:_docmentFilePath];
    }
}
