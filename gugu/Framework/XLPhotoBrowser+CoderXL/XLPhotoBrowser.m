...
@property (strong, nonatomic) UIView *viewNav;
/**
 *  保存按钮
 */
...
- (void)iniaialUI
{
    ...
    _viewNav = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NavBarHeight-20)];
    _viewNav.backgroundColor =[[UIColor blackColor] colorWithAlphaComponent:0.618];
    [self addSubview:_viewNav];
    
    // 添加XLPhotoBrowserStyleSimple相关控件
    UILabel *indexLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(_viewNav.frame)-44, SCREEN_WIDTH, 44)];
    indexLabel.textAlignment = NSTextAlignmentCenter;
    indexLabel.textColor = [UIColor whiteColor];
    indexLabel.font = [UIFont systemFontOfSize:18];
    indexLabel.clipsToBounds = YES;
    self.indexLabel = indexLabel;
    [_viewNav addSubview:indexLabel];
    
    //暂时隐藏
    UIButton *saveButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [saveButton setImage:[UIImage imageNamed:@"down"] forState:UIControlStateNormal];
    saveButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
    saveButton.frame = CGRectMake(SCREEN_WIDTH-60, CGRectGetMidY(indexLabel.frame)-22, 44, 44);
    [saveButton addTarget:self action:@selector(saveImage) forControlEvents:UIControlEventTouchUpInside];
    self.saveButton = saveButton;
    [_viewNav addSubview:saveButton];
    
    UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [backBtn setImage:[UIImage imageNamed:@"return_white"] forState:UIControlStateNormal];
    backBtn.frame = CGRectMake(15, CGRectGetMidY(indexLabel.frame)-22, 44, 44);
    [backBtn addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];
    backBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    [_viewNav addSubview:backBtn];
...
- (void)updateFrames
{
    ...
    [self  bringSubviewToFront:_viewNav];
...
- (void)updateIndexVisible
{
    ...
    self.indexLabel.hidden = _saveButton.hidden = NO;
}
