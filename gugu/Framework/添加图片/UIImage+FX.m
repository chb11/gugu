...
- (UIImage *)imageWithButtomCornerRadius:(CGFloat)radius
{
    ...
    CGContextAddLineToPoint(context, self.size.width - radius, self.size.height);
    CGContextAddArc(context, self.size.width - radius, self.size.height - radius, radius, M_PI / 2.0f, 0.0f, 1);
    CGContextAddLineToPoint(context, self.size.width, 0.0f);
    CGContextAddLineToPoint(context, 0.0, 0.0f);
    CGContextClip(context);
...
- (UIImage *)maskImageFromImageAlpha
{
    ...
    CGContextRef context = CGBitmapContextCreate(data, width, height, 8, bytesPerRow, NULL, 7);
