...
@interface CB_ActivityChatController ()<SSChatKeyBoardInputViewDelegate,UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,SSChatBaseCellDelegate>
...
@property (nonatomic,strong) SSSearchmapView *mapView;
...
@implementation CB_ActivityChatController
...
-(void)viewDidLayoutSubviews{
    ...
    self.mapView.keywordView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 40);
}
...
-(void)initUI{
    ...
    self.mapView = [[SSSearchmapView alloc] initWithFrame:self.view.bounds];
...
-(void)initAction{
    ...
    self.mapView.block_chooseAddress = ^(MAPointAnnotation * _Nonnull annotaion) {
        [waekSelf action_changeTargetToServerWith:annotaion];
    };
    self.mapView.block_userCount = ^(NSInteger count) {
        waekSelf.title = [NSString stringWithFormat:@"%ld人",count+1];
    };
}
...
-(void)action_userPostitionUpdate:(NSNotification *)notification{
    ...
    if ([model.ActivityId isEqualToString:self.activitGroupModel.Guid]) {
        [self.mapView action_updateUserPositionWith:model];
    }
...
//    }];
    ...
    NSArray *menuModelArr = @[menuModel1,menuModel2,menuModel3];
    return menuModelArr;
}
